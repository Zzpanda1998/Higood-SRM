...
export const menuGroups = [
  ...
  { title: "原料管理", items: ["面辅料库存监控"] },
  { title: "采购建议", items: ["商品采购建议", "KOL采购需求"] },
  { title: "商品采购", items: ["商品采购单"] },
  {
    title: "面辅料采购",
    items: ["面辅料需求分析", "面辅料采购单", "面辅料采购跟踪"],
  },
  { title: "面辅料供应商确认", items: ["面辅料供应商确认单"] },
...
function MenuButton({
  ...
  return (
    ...
    >
      {isActive && (
        <span className="absolute left-0 top-1.5 h-5 w-0.5 rounded-full bg-brand" />
      )}
      {item}
    </button>
  );
  ...
  return (
    <aside className="grid h-full w-64 shrink-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border-r border-gray-200 bg-white">
      <div className="border-b border-gray-100 p-3">
        ...
        <input
          className="h-8 w-full rounded border border-gray-200 px-2 text-sm outline-none focus:border-brand"
          placeholder="搜索菜单..."
        />
      </div>
      ...
      <nav className="overflow-y-auto overflow-x-hidden px-3 py-2">
        ...
        {menuGroups.map((group, index) => (
          <section
            key={group.title}
            className={index === 0 ? "mb-3" : "mb-3 mt-3"}
          >
            <div className="mb-1.5 truncate px-2 text-[13px] font-semibold leading-6 text-gray-900">
              {group.title}
            </div>
