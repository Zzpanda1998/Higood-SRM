...
export default function IdMaterialPurchaseOrder({
  ...
  return (
    <div>
      ...
      <DesignLogicCard sections={[
        { title: "贸易与销售信息页面功能说明", headers: ["说明"], rows: [["采购单中的贸易与销售信息用于记录香港公司采购后对外销售的真实业务数据。销售对象不通过固定链路配置决定，而是在采购单中从贸易主体管理手动选择。销售价格和销售金额由业务或财务录入，后续主体经营明细直接读取采购单中的最终销售对象、销售金额和收入状态。"]] },
        { title: "贸易与销售业务逻辑说明", headers: ["业务场景", "规则说明", "页面结果"], rows: [["销售对象不固定", "不按物料类型限制销售对象", "采购单中手动选择销售对象"], ["销售金额手动", "不做自动加价规则", "财务或业务人员录入销售金额"], ["收入确认待定", "暂不做强制收入确认流程", "只展示收入状态字段"], ["数据进入经营明细", "采购单保存后进入主体经营明细", "老板和财务可查看经营数据"], ["销售金额缺失", "销售金额为空时标记未录入", "主体经营明细可筛选缺失项"], ["成本缺失", "部分费用未录入或未分摊时标记不完整", "主体经营明细可筛选成本不完整记录"]] },
      ]} />
      <FormModal open={Boolean(detailOrder)} title={`面辅料采购单详情 ${detailOrder?.idOrderNo ?? ""}`} widthClass="w-[980px]" onClose={() => setDetailOrder(null)}>
        {detailOrder && (
          <div className="space-y-3 text-sm">
            ...
            <section className="rounded border border-blue-200 bg-blue-50/40 p-3"><h3 className="mb-3 font-semibold text-blue-900">贸易与销售信息</h3><div className="grid gap-3 text-xs md:grid-cols-4"><label className="grid gap-1 text-gray-500">采购主体<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>HiGOOD 香港公司</option></select></label><label className="grid gap-1 text-gray-500">出口主体<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>HiGOOD 香港公司</option></select></label><label className="grid gap-1 text-gray-500">物料类型<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>面料</option><option>辅料</option><option>办公耗材</option><option>机器设备</option><option>固定资产</option></select></label><label className="grid gap-1 text-gray-500">国内供应商<div className="h-8 rounded border bg-gray-50 px-2 leading-8 text-gray-800">{detailOrder.supplier}</div></label><label className="grid gap-1 text-gray-500">销售对象<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>PT CM</option><option>GOTO</option><option>FADFAD</option><option>印尼车缝工厂 A</option></select></label><label className="grid gap-1 text-gray-500">销售币种<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>CNY</option><option>USD</option><option>IDR</option></select></label><label className="grid gap-1 text-gray-500">销售单价（手动录入）<input className="h-8 rounded border bg-white px-2 text-gray-800" defaultValue="13.00" /></label><label className="grid gap-1 text-gray-500">销售金额（可手动覆盖）<input className="h-8 rounded border bg-white px-2 text-gray-800" defaultValue="130000.00" /></label><div><span className="text-gray-500">销售金额录入状态</span><div className="mt-1"><span className="rounded-full bg-emerald-50 px-2 py-1 text-emerald-700">已录入</span></div></div><label className="grid gap-1 text-gray-500">收入状态<select className="h-8 rounded border bg-white px-2 text-gray-800"><option>待确认</option><option>未确认</option><option>已确认</option><option>已收款</option></select></label><label className="grid gap-1 text-gray-500">收入确认时间<input className="h-8 rounded border bg-white px-2" type="datetime-local" /></label><div><span className="text-gray-500">成本是否完整</span><div className="mt-1"><span className="rounded-full bg-emerald-50 px-2 py-1 text-emerald-700">完整</span></div></div></div><label className="mt-3 grid gap-1 text-xs text-gray-500">销售备注<textarea className="h-16 rounded border bg-white px-2 py-1" defaultValue="销售对象与销售金额按本次真实业务手动录入。" /></label><p className="mt-2 text-xs text-gray-500">销售对象不按物料类型固定；销售金额以人工最终确认值为准，不执行固定加价公式。</p></section>
          </div>
        )}
      </FormModal>
