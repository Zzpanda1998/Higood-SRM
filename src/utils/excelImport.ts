...
const parseHtmlTable = (text: string) => {
  ...
  return Array.from(doc.querySelectorAll("tr")).map((row) => Array.from(row.querySelectorAll("th,td")).map((cell) => cell.textContent?.trim() ?? "")).filter((row) => row.length);
};

const unzipEntry = async (bytes: Uint8Array, entryName: string) => {
...
const parseWorksheet = (xml: string, shared: string[]) => {
  const doc = new DOMParser().parseFromString(xml, "application/xml");
...
export const parseExcelRows = async (file: File): Promise<string[][]> => {
